function [yln y logSqrtDetSigma] = mvnpdfln(X, Mu, Sigma, isPrec)
...
if nargin<4
    isPrec=0; %default covariance instead of precision matrix
end
    ...
    elseif ~isequal(sz, [d d])
        ...
        if sigmaIsDiag
            ...
            if ~isPrec
                ...
                xRinv = bsxfun(@times,X0*X0',Sigma);
                logSqrtDetSigma = 0.5*logdetMatlab(Sigma);
            end
        else
...
%                 end
                ...
                xRinv = X0 * Sigma * X0'; %since X0 are row vectors
                logSqrtDetSigma = .5*logdetMatlab(Sigma);
            end
        end
    end
...
%                     end
                ...
                xRinv = zeros(n,1);
                for i = 1:n
                    xRinv(i,:) = X0(i,:) * squeeze(Sigma(:,:,i)) * X0(i,:)';
%                     logSqrtDetSigma(i) = 0.5*logdetMatlab(squeeze(Sigma(:,:,i)));
                    logSqrtDetSigma(i) = 0.5*logdetMatlab(Sigma(:,:,i));
                end
            end
        end
    end
...
if ~isPrec
    ...
    yln = -0.5*quadform - logSqrtDetSigma - d*log(2*pi)/2;
else
%     if size(X0,2)>1
%         quadform = diag(xRinv); %already squared form
%     else
        quadform = xRinv; %already squared form
%     end
    yln = -0.5*quadform + logSqrtDetSigma - d*log(2*pi)/2;
end
y = exp(yln);