...
#
# functions written by : Dimitrios D. Thomakos, Dimitris N.Politis, Jie Chen
# 
# updated 6/23/2015
#
...
######################### Get the transformed series w_t by NoVaS simple ######### 
...
########Algorithm of exponential NoVaS.
...
P.OPTexpoCCcorrected.robust<- function(x, CCC = 0, refine = T, plot = F)
{ # x is financial returns series (percentage returns)
  ...
  for(i in 1:100) {
    zz[i] <- KURTOSIS(VARSTABexpoCCcorrected(x, d = ccc[i], cc = CCC))
  }
...
###################### prediction one-step-ahead of single volatility (not the whole series)
...
{  # xx is financial returns series (percentage returns)
  ...
  w2 <- ww^2/(1-a0*ww^2)
  m1 <- median(w2[(p+1):n], na.rm = T)# for L2 method, m1<- mean(w2[(p+1):n], na.rm = T)
  one_step_pre <- m1*An2
  one_step_pre
}
...
######### Table 4 The L1-comparison of predictors for GARCH with normal errors and t-errors. 
####Input the required data
dataset<-as.matrix(read.csv("3timeseries.csv"))
...
############### Table 6&7. Generalized Simple NoVaS with parameter alpha ##########
####Input the required data
dataset<-as.matrix(read.csv("3timeseries.csv"))
...
##################################################################################################################
################# Simulation in section 10.7 Time-varing NoVaS: robustness against structural breaks ######
